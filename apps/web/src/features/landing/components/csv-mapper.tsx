  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <select
        ...
        style={MONO}
        className="bg-background border border-border rounded-md px-2.5 py-2 text-[12px] text-foreground focus:outline-none focus:border-border-strong transition-colors"
      >
...
export function CsvMapper({
  ...
  return (
    <div className="p-4 sm:p-5">
      ...
      <div
        style={MONO}
        className="text-hint text-[10px] tracking-[0.16em] uppercase mb-2"
      >
        preview
      </div>
      {convs.length === 0 ? (
        <p style={MONO} className="text-subtle text-[11px] py-4">
          ...
          {convs.map((c, i) => (
            ...
            >
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-border bg-muted/40">
                <span style={MONO} className="text-subtle text-[10.5px]">
              ...
              <div className="px-3 py-2 space-y-1.5">
                {c.messages.slice(0, 6).map((m, j) => (
                  ...
                  >
                    ...
                    >
                      <span
                        style={MONO}
                        className={`block text-[9px] tracking-[0.1em] uppercase mb-0.5 ${m.role === 'assistant' ? 'text-background/55' : 'text-hint'}`}
                      >
                ...
                {c.messages.length > 6 && (
                  <p style={MONO} className="text-hint text-[10px] pt-0.5">
      ...
      <div className="mt-5 flex items-center gap-2.5">
        ...
        <button
          onClick={onCancel}
          style={MONO}
          className="text-hint hover:text-foreground transition-colors text-[11px]"
        >
          cancel
        </button>
      </div>
